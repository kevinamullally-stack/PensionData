...
| `asset_class_returns_json` | Optional. A JSON string of any per-asset-class actual-vs-benchmark return pairs disclosed in the same table, e.g. `{"us_equity": {"actual": 8.2, "benchmark": 7.9}, "fixed_income": {"actual": 4.1, "benchmark": 3.9}}`. Blank if the source only reports total-fund-level returns. |
